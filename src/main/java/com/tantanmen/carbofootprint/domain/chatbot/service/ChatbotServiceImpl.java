...
public class ChatbotServiceImpl implements ChatbotService {
    ...
    public String setPromptAboutInfo(String question) {
        ...
        prompt += "{"
                + "\"model\": \"" + model +"\","
                + "\"messages\": [{\"role\": \"user\", \"content\": \"" + "너는 탄수화물 중독 관리 서비스의 챗봇이야. 모든 질문에 대한 답변을 챗봇 형식으로 간단하게 300자 이내로 정리해서 알려줘. (과일도 탄수화물이야? 라는 질문에 대한 예시 :  네, 건강식품으로 꼽히는 과일도 탄수화물 음식의 일종입니다! 탄수화물 함량이 적은 과일로는 [복숭아], [사과], [수박] 등이 있습니다!) 질문은 다음과 같아. " + question + "\"}]"
                + "}";
    ...
    public String setRecommendPrompt() {
        ...
        prompt += "{"
                + "\"model\": \"" + model +"\","
                + "\"messages\": [{\"role\": \"user\", \"content\": \"" + "너는 탄수화물 중독 관리 서비스의 챗봇이야. 탄수화물 중독 위험에서 벗어난 건강한 외식 메뉴 하나 추천해줘. 음식 이름은 [음식이름]으로 [ ]로 감싸서 답해줘. (답변 예시 : 탄수화물 중독 위험에서 벗어난 건강한 외식 메뉴를 추천해드리겠습니다! [그릴드 연어와 아스파라거스]는 어떠신가요?)" + "\"}]"
                + "}";
